...
export const PutOne = async(ctx, next) => {
  ...
  let user = await ctx.models.User.findById(ctx.params.id)
  if (!user) throw new APIError(404, ErrorsCategory[4], 'user doesnot esit')
  ...
  const city = ChinaAreaData[ctx.request.body.province][ctx.request.body.city]
  const country = ChinaAreaData[ctx.request.body.city][ctx.request.body.country]
