<template>
  <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120">
    ...
    <Form-item label="对话食咖" prop="markdown_shika">
...
<script>
  ...
  export default {
    ...
    async created () {
      this.formValidate.restaurant_id = this.$route.params.id
    ...
    methods: {
      ...
      async handleSubmit (formValidate) {
        const body = {
          ...
          cover: this.formValidate.uploadList2[0].dataUrl,  // 2017-11-16 old:this.formValidate.avatar
          ...
          recommend_id: this.formValidate.recommend_id.join(),
          specialty_imgs: this.formValidate.uploadList2    // 2017-11-16
        }
        this.$refs[formValidate].validate(async (valid) => {
          if (valid) {
            const res = await this.axios.post('/specialty', body)
      ...
      handleSuccess2 (res, file) {
        ...
        this.formValidate.avatar = this.formValidate.uploadList2[0].dataUrl
      },
      ...
      handleBeforeUpload2 () {
        const check = this.formValidate.uploadList2.length < 2  // 2017-11-16
        if (!check) {
            this.$Notice.warning({
                title: '最多只能上传 2 张图片。'
            })
      ...
      handleClose (index) { // 推荐餐厅删除事件处理
        ...
        this.formValidate.recommend_id.splice(index, 1)
        console.log('=========删除推荐餐厅数据成功=======' + this.formValidate.recommend_id)
      },
