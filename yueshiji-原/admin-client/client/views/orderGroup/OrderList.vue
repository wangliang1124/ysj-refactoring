<template>
  <div>
    <Input v-model="q" placeholder="搜索团购订单" @on-change='searchUser'>
...
<script>
  ...
  export default {
              ...
              return h('div', [
                h('Button', {
                  props: {
                    type: 'primary',
                    size: 'small',
                    disabled: params.row.status.status === '1'
                  },
                  style: {
                    marginRight: '5px'
                  },
                  on: {
                    click: async () => {
                      const res = await this.axios.put(`/group/order/${params.row.id}`)
    ...
    exportData () {
      const data = {
        name: `ordergroup-${this.page}.xlsx`,
      ...
      async initData () {
        const url = `/group/order?order=${this.order}&sortby=${this.sortby}&per_page=${this.per_page}&page=${this.page}&filter=${this.filter}&q=${this.q}&field=${this.field}`
