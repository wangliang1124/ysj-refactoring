...
<script>
  export default {
    ...
    async created () {
      ...
      this.formItem.citys_area = res2.data
    },
    methods: {
      async initData () {
      ...
      async submit () {
        ...
        if (content) {
          ...
          const res = await this.axios.post('/card', this.formItem)
          if (res.message && res.message === 'success') this.$router.push({path: '/card', reload: true})
        }
      }
    }
  }
